...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem left; // controls whether the text is italic or not.
	...
	public TextMenu(DrawPanel owner) {
		...
		/**
		 * 6. Use an Input Dialog
		 *  support for changing the value of the lineHeightMultiplier 
		 */
		...
		lineSpacing.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				String s = JOptionPane.showInputDialog(panel, "What line spacing do you want to use?",currentSize);
				if (s != null && s.trim().length() > 0) {
					try {
						...
						JOptionPane.showMessageDialog(panel, s + " is not a legal spacing size.\n"
								+"Please enter a positive integer.");
					}
				}
			}
	...
	/**
	 * 5. Add Radio Buttons in a Submenu
	 * Create a menu containing a list of text justify options.
	 */
