...
public class GuiDemo extends JFrame{
	...
	/**
	 * 2: Make a Toolbar
	 * Create the "Toolbar" menu, using objects of type ChooseBackgroundAction,
	 * a class that is defined later in this file.
	 */
	private JToolBar makeToolbar() {
		JToolBar topToolBar = new JToolBar(JToolBar.HORIZONTAL);
		topToolBar.add(newPictureAction);		
		topToolBar.add(saveImageAction);
		...
		topToolBar.addSeparator();
		topToolBar.add(customPictureAction);
		topToolBar.add(new colorAction());
		topToolBar.addSeparator();
	...
	/*
	 * 3: Adding an Icon to an Action
	 * */
	private AbstractAction customPictureAction = 
			new AbstractAction("Custom", Util.iconFromResource("resources/action_icons/fileopen.png")) {
			    public void actionPerformed(ActionEvent evt) {			    	
						File inputFile = fileChooser.getInputFile(drawPanel, "Select Background Image");
						putValue(Action.SHORT_DESCRIPTION, "Select a file for the background");
						if (inputFile != null) {
							try {
	...
	private AbstractAction saveImageAction = 
    	...
    	/*
    	 * 4: Create an ImageIcon from Scratch for users to select a custom color 
    	 * */
    	...
    	private class colorAction extends AbstractAction {
    		colorAction() {
    			...
    			g.fillRect(10, 1, 10, 80);
    			g.setColor(Color.BLUE);
    			g.fillRect(1, 1,10, 80);
    			g.setColor(Color.GREEN);
    			g.fillRect(20, 1,10, 80);
    			g.dispose();
    			putValue(Action.SMALL_ICON, new ImageIcon(col));
    			putValue(Action.SHORT_DESCRIPTION, "Choose custom colors"); // tooltip			
    		}
    		public void actionPerformed(ActionEvent evt) {
